...
class ActorNetwork(object):
    ...
    def create_actor_network(self):
        ...
        with tf.variable_scope(self.vscope):
            ...
            w1 = tf.Variable(tf.truncated_normal(shape=[self.s_dim,400],name="aw1"))
            b1 = tf.Variable(tf.zeros([400],name="ab1"))
            w2 = tf.Variable(tf.truncated_normal(shape=[400,300],name="aw2"))
            b2 = tf.Variable(tf.zeros([300],name="ab2"))
            w3 = tf.Variable(tf.truncated_normal(shape=[300,self.a_dim],name="aw3"))
            b3 = tf.Variable(tf.zeros([self.a_dim],name="ab3"))


            net1 = tf.nn.relu(tf.matmul(inputs, w1)+b1,name="anet1")
            net2 = tf.nn.relu(tf.matmul(net1,w2)+b2,name="anet2")
            out = tf.nn.tanh(tf.matmul(net2,w3) +b3,name="aout")
...
class CriticNetwork(object):
    ...
    def create_critic_network(self):
        ...
        with tf.variable_scope(self.vscope):
            ...
            w2 = tf.Variable(tf.truncated_normal(shape=[400, 300], name="aw2"))
            b2 = tf.Variable(tf.zeros([300], name="ab2"))
            w3 = tf.Variable(tf.truncated_normal(shape=[ self.a_dim,300], name="aw3"))
            b3 = tf.Variable(tf.zeros([300], name="ab3"))
            w4 = tf.Variable(tf.random_uniform([300,1],minval=-0.003, maxval=0.003))
            net1 = tf.nn.relu(tf.matmul(inputs, w1) + b1, name="anet1")
            net2 = tf.nn.relu(tf.matmul(net1, w2) + tf.matmul(action,w3)+ b3, name="anet2")
            out = tf.matmul(net2, w4)
